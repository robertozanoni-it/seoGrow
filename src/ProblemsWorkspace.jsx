...
const statusLabel = (value) => {
  ...
  if (/ripristinat/.test(text)) return "Da fare";
  return "Da fare";
};
...
const auditProblem = (issue, analyzedAt) => {
  ...
  return {
    ...
    status: "Da fare",
    source: "Audit SeoGrow",
    sourceKind: "audit",
    sourceUrl,
    detail: issue.detail || "Problema rilevato nell’ultimo audit del progetto.",
    observedAt: analyzedAt || "",
    fields: [],
  };
};

const correctionProblem = (record) => ({
  key: record.issueKey || stableIssueKey(record),
  title: record.issueLabel || record.issueType || "Correzione SEO",
  issueType: record.issueType || "",
  severity: severityLabel(record.severity),
  status: statusLabel(record.status),
  source: "Correzioni WordPress",
  sourceKind: "correction",
  sourceUrl: record.sourceUrl || "",
  detail:
    record.status === "Verificato"
      ? "La correzione è stata applicata e verificata sul sito."
      : record.status === "Applicato"
        ? "La modifica è stata scritta in WordPress e deve essere verificata sul frontend."
        : "Correzione registrata nello storico SeoGrow.",
  observedAt: record.verifiedAt || record.appliedAt || record.rollbackAt || "",
  fields: Array.isArray(record.fields) ? record.fields : [],
  correctionStatus: record.status || "",
});

const taskProblem = (task) => {
  ...
  return {
    ...
    title: task.title || "Task SEO",
    issueType: task.kind || "task",
    severity: severityLabel(task.priority),
    status: statusLabel(task.status),
    source: "Task SeoGrow",
    sourceKind: "task",
    sourceUrl,
    detail: task.detail || task.notes || "Attività generata dai controlli del progetto.",
    observedAt: task.updatedAt || task.createdAt || "",
    fields: [],
    taskId: task.id,
  };
};
...
function buildProblems(clientId, clientName) {
  ...
  const analysis = latestAnalysis(analyses[clientId]);
  const auditRows = (analysis?.issues || []).map((issue) =>
    auditProblem(issue, analysis?.analyzedAt),
  );
  const corrections = readJson(REMEDIATION_INDEX_KEY, []).filter(
    (record) => Number(record.clientId) === Number(clientId),
  );
  const correctionRows = corrections.map(correctionProblem);
  const taskRows = readJson(TASKS_KEY, [])
    .filter(
      (task) =>
        (task.sourceClientId === clientId ||
          (!task.sourceClientId && task.client === clientName)) &&
        (String(task.id || "").startsWith("analysis-") ||
          task.kind === "broken-link" ||
          task.regression),
    )
    .map(taskProblem);
    ...
    merged.set(
      ...
      previous
        ? {
            ...previous,
            ...row,
            detail: previous.detail || row.detail,
            source: `${previous.source} · ${row.source}`,
          }
        : row,
    );
  }
...
function ProblemDrawer({ problem, onClose }) {
  ...
  const originPage =
    problem.sourceKind === "correction"
      ? "Correzioni"
      : problem.sourceKind === "task"
        ? "Task"
        : "Audit SEO";
  return createPortal(
    <>
      ...
      <aside className="problem-drawer" aria-label={`Dettaglio: ${problem.title}`}>
        <div className="problem-drawer-head">
          <div>
            <span className={`problem-severity ${problem.severity.toLowerCase()}`}>{problem.severity}</span>
            <h2>{problem.title}</h2>
          </div>
        ...
        <div className="problem-drawer-status">
          <span className={`problem-status ${problem.status.toLowerCase().replaceAll(" ", "-")}`}>
            {problem.status === "Verificato" ? <CheckCircle2 /> : <AlertTriangle />}
            {problem.status}
          </span>
        ...
        <section>
          <h3>Problema ed evidenze</h3>
          <p>{problem.detail || "Dettaglio non disponibile."}</p>
          {problem.fields.length > 0 && (
            <div className="problem-field-list">
              <strong>Campi coinvolti</strong>
              <span>{problem.fields.join(", ")}</span>
            </div>
          )}
        </section>

        <section className="problem-why">
          <h3><Info /> Perché lo vedo?</h3>
          <dl>
            <div><dt>Fonte</dt><dd>{problem.source}</dd></div>
            <div><dt>Rilevato/aggiornato</dt><dd>{formatDate(problem.observedAt)}</dd></div>
            <div><dt>Stato</dt><dd>{problem.status}</dd></div>
          </dl>
        </section>

        {problem.sourceUrl && (
          <a className="secondary problem-resource" href={problem.sourceUrl} target="_blank" rel="noreferrer">
            <ExternalLink /> Apri la pagina interessata
          </a>
        )}

        <div className="problem-next-step">
          <span>Prossima azione</span>
          <button className="primary" onClick={() => navigate(originPage)}>
            {problem.status === "Da verificare" ? "Verifica ora" : problem.status === "Verificato" ? "Vedi dettaglio" : "Apri intervento"}
          </button>
          <button className="secondary" onClick={() => navigate("SEO Agent")}>
            <Sparkles /> Chiedi a SeoGrow
          </button>
        </div>
      </aside>
    </>,
...
export default function ProblemsWorkspace() {
  ...
  useEffect(() => {
    const refresh = () => {
      setActive(currentHash() === "Problemi");
  ...
  useEffect(() => {
    ...
    if (active) mainTarget.dataset.problemsOpen = "true";
    else delete mainTarget.dataset.problemsOpen;
    return () => { delete mainTarget.dataset.problemsOpen; };
  }, [active, mainTarget]);
  ...
  const rows = useMemo(
    () => buildProblems(client?.id, client?.name),
    [client?.id, client?.name, active],
  );
  ...
  const filtered = rows.filter((row) => {
    if (filter === "Critici") return row.severity === "Critico" && row.status !== "Verificato";
    if (filter === "Da fare") return row.status === "Da fare" || row.status === "In lavorazione";
    if (filter === "Da verificare") return row.status === "Da verificare";
    if (filter === "Verificati") return row.status === "Verificato";
    return true;
  });
    ...
    <div className="problems-workspace-root">
      <div className="page-title problems-title">
        <div>
          ...
          <h1>Problemi — {client?.name || "progetto"}</h1>
          <p>Audit, Task tecniche e Correzioni raccolti in un’unica vista. Apri un problema per capire fonte, evidenze e prossima azione.</p>
        </div>
        <div className="problems-view-toggle" aria-label="Tipo di visualizzazione">
          <button className={view === "compact" ? "active" : ""} onClick={() => setView("compact")}><LayoutList /> Compatta</button>
          <button className={view === "detailed" ? "active" : ""} onClick={() => setView("detailed")}><ListTree /> Dettagliata</button>
        </div>
      </div>
      ...
      <div className="problems-toolbar">
        ...
        <span className="problems-source-note"><ShieldCheck /> Stati derivati da Audit, Task e storico Correzioni già presenti in SeoGrow.</span>
      </div>

      <section className={`problems-list ${view}`}>
        {filtered.length ? filtered.map((problem) => (
          <button className="problem-row" key={problem.key} onClick={() => setSelected(problem)}>
            <span className={`problem-severity ${problem.severity.toLowerCase()}`}>{problem.severity}</span>
            <span className="problem-main">
              <strong>{problem.title}</strong>
              <small>{problem.sourceUrl || "URL non disponibile"}</small>
              {view === "detailed" && <p>{compactText(problem.detail)}</p>}
            </span>
            <span className="problem-source"><FileSearch /> {problem.source}</span>
            <span className={`problem-status ${problem.status.toLowerCase().replaceAll(" ", "-")}`}>{problem.status}</span>
            <ChevronRight className="problem-chevron" />
          </button>
        )) : (
          <div className="problems-empty">
            <CheckCircle2 />
            <h2>Nessun problema in questo filtro</h2>
            <p>Cambia filtro oppure esegui un nuovo audit per aggiornare la situazione.</p>
            <button className="secondary" onClick={() => navigate("Audit SEO")}><CircleGauge /> Apri Audit SEO</button>
          </div>
        )}
      </section>
